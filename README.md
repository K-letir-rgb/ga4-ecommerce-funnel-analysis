...
End-to-end analysis of the GA4 obfuscated sample e-commerce dataset (Google's public BigQuery dataset), examining user conversion behavior through the purchase funnel for November 2020.

**Tools:** BigQuery (SQL) → Python (pandas) → Power BI
...
[View full dashboard (PDF)](outputs/charts/ga4_funnel_dashboard.pdf)

The dashboard visualizes the funnel drop-off, checkout path volume split, and purchase rate comparison shown above.

## Methodology

- Queried `bigquery-public-data.ga4_obfuscated_sample_ecommerce.events_*` in BigQuery, filtered to November 2020 (`_TABLE_SUFFIX BETWEEN '20201101' AND '20201130'`)
- Counted raw event occurrences per funnel stage, then re-ran at the unique-user level (`COUNT(DISTINCT user_pseudo_id)`) — raw counts overstate stage size due to repeat events per user
- Built a per-user flag table (`MAX(event_name = 'X')` pattern) to classify users by which events they fired, then segmented checkout-reaching users by cart usage and calculated purchase rate per segment
- Cleaned and reshaped exported CSVs in Python (pandas) ahead of dashboard build
- Built funnel and checkout-path visualizations in Power BI: drop-off funnel chart, user volume by path, and purchase rate by path

SQL queries are in `/sql`.

## Status

✅ Complete — BigQuery SQL, Python cleaning, and Power BI dashboard all finished and pushed.
